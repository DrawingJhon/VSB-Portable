local _G, game, script, getfenv, setfenv, workspace, getmetatable, setmetatable, loadstring, coroutine, rawequal, typeof, print, math, warn, error,  pcall, xpcall, select, rawset, rawget, ipairs, pairs, next, Rect, Axes, os, time, Faces, unpack, string, Color3, newproxy, tostring, tonumber, Instance, TweenInfo, BrickColor, NumberRange, ColorSequence, NumberSequence, ColorSequenceKeypoint, NumberSequenceKeypoint, PhysicalProperties, Region3int16, Vector3int16, elapsedTime, require, table, type, wait, Enum, UDim, UDim2, Vector2, Vector3, Region3, CFrame, Ray, spawn, delay, task, bit32 = _G, game, script, getfenv, setfenv, workspace, getmetatable, setmetatable, loadstring, coroutine, rawequal, typeof, print, math, warn, error,  pcall, xpcall, select, rawset, rawget, ipairs, pairs, next, Rect, Axes, os, time, Faces, unpack, string, Color3, newproxy, tostring, tonumber, Instance, TweenInfo, BrickColor, NumberRange, ColorSequence, NumberSequence, ColorSequenceKeypoint, NumberSequenceKeypoint, PhysicalProperties, Region3int16, Vector3int16, elapsedTime, require, table, type, wait, Enum, UDim, UDim2, Vector2, Vector3, Region3, CFrame, Ray, spawn, delay, task, bit32;
local _remote = script:WaitForChild("Source")
local _source = _remote:InvokeServer()

if getmetatable(shared) == "The metatable is locked (from ox-side)" then
	shared(script)
	spawn(function() local d = _remote:WaitForChild("Manager", 9e5) if d then d:Destroy() end end)
else
	require(_remote:WaitForChild("Manager"))(script)
end
...
local compile = Yueliang()
local createExecutable = FiOne()

wait()

local function GetExecute()
	local executable
	local env = getfenv(0)
	local name = "[string \"SB_Script\"]"
	local ran, failureReason = pcall(function()
		local compiledBytecode = compile(_source, name)
